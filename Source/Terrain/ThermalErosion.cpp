...
namespace terrain_ops
{
    ...
    void ThermalErosion::operate(HeightmapStack* stack, Progress* progress)
    {
                ...
                for (int y = 0; y < h->GetHeight(); ++y)
                {
                    ...
                    for (int i = 0; i < NUM_NEIGHBOURS; ++i)
                    {
                        if (h->IndexValid(nIndices[i][0], nIndices[i][1]))
                        {
                            ...
                            dis[i] = h->GetScaledHeight(x, y) - h->GetScaledHeight(nIndices[i][0], nIndices[i][1]);
                    ...
                    for (int i = 0; i < NUM_NEIGHBOURS; ++i)
                    {
                        if (isValid[i])
                        {
                            (*h)(nIndices[i][0], nIndices[i][1]) += ((c * (dmax - talusAngle)) * (dis[i] / dtotal)) / h->GetScaleY();
                        }
                    }
                }
            }
        ...
        cout << "Thermal Erosion Time(Ms): " << timer.Time() << endl;
        stack->push(h);
    }

    void ThermalErosion::inspect(IPropertyInspector& inspector)
    {
